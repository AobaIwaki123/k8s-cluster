...
**採用方針: 公開アプリNSに「他NSへのEgress拒否のみ」を設定する**

Cloudflare Tunnelからの着信はIngressであるためEgress拒否の影響を受けず、そのまま動作する。

```
Cloudflare Network
    ↓
controlled-cloudflared-connector Pod (cloudflare-tunnel-ingress-controller NS)
    ↓ ClusterIP Service経由（Ingress = 着信）
journee Pod  ← Egress拒否は着信に影響しない
    ↓ ← ここを拒否する（他NSへの発信）
observability NS / smart-home NS 等
```
...
- **公開アプリNSへの適用を推奨する**
- 「ネットワークに到達できるだけでは脅威度は低い」が、**認証なしで動いている内部サービス（Prometheus等）が存在する場合は実害がある**
- NetworkPolicyはkube-routerなどCNIがサポートしている場合のみ有効（このクラスターはkube-router使用、対応済み）
- **Cloudflare Tunnelとの兼ね合い**: `controlled-cloudflared-connector` はService経由でアプリPodに到達する（Ingress = 着信）。Egress拒否はアプリからの発信を制御するものなので、Cloudflare側からの着信には影響しない
